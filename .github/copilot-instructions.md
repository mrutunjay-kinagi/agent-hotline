...
- Multiple terminals coordinate: Builder (local model), QA (cloud), DevOps (cloud) all working on the same codebase via `.hotline.json`.
...
- The intended workflow is: Builder codes locally → `agent-hotline write` handoff → QA tests thoroughly → DevOps deploys safely.
...
- Keep the Builder → QA → DevOps role sequence intact when proposing workflow changes.
- Builder should always run tests locally before handing off; QA should verify thoroughly before DevOps deploys.
